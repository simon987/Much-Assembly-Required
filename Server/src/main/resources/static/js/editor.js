...
function parseInstruction(line, result, currentLine) {
    ...
    if (!parseDWInstruction(line, result, currentLine)) {
        ...
        if (new RegExp('\\b(?:mov|add|sub|and|or|test|cmp|shl|shr|mul|push|pop|div|xor|hwi|hwq|nop|neg|' +
            ...
            'call|ret|jmp|jnz|jg|jl|jge|jle|int|jz|js|jns|brk|not|jc|jnc|ror|rol|sal|sar|jo|jno|inc|dec|rcl|xchg|rcr|pushf|popf|ja|jna' +
            'jne|je|jnle|jnl|jnge|jng|jae|jnb|jb|jnae|jnbe|jbe)\\b').test(mnemonic.toLowerCase())) {
            ...
            if (line.indexOf(",") !== -1) {
                ...
                //Validate operand number
                if (!new RegExp('\\b(?:push|mul|pop|div|neg|call|jnz|jg|jl|jge|jle|hwi|hwq|jz|js|jns|ret|jmp|not|jc|jnc|jo|jno|inc|dec|ja|jna|seta|setnbe|setae|setnb|setnc|setbe|setna|setb|setc|setnae|sete|setz|setne|setnz|setg|setnle|setge|setnl|setle|setng|setl|setnge|seto|setno|sets|setns|jne|je|jnle|jnl|jnge|jng|jae|jnb|jb|jnae|jnbe|jbe)\\b').test(mnemonic.toLowerCase())) {
                    result.annotations.push({
                        row: currentLine,
                        column: 0,
